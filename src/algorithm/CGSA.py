...
class CGSAOptimizer(BaseOptimizer, torch.optim.Optimizer):
    def __init__(self, params, **kwargs):
        self.lr = kwargs.get('lr')
        self.momentum = kwargs.get('momentum', 0.)
        defaults = dict(lr=self.lr, momentum=self.momentum)
        super(CGSAOptimizer, self).__init__(params=params, defaults=defaults)
    ...
    def accumulate(self, mixing_coefficient, local_param_iterator, partial_agg_condition=lambda name: None):
        for group in self.param_groups:
            for server_param, (name, local_param) in zip(group['params'], local_param_iterator):
                ...
                ori_shape = vec.shape
                tensor = vec.view(-1)
                max_items = math.ceil(torch.abs(tensor).numel()*32*0.032)
                mask= self.simulated_annealing(torch.abs(tensor), max_items)
                vec_q= torch.zeros_like(tensor)
                vec_q[mask==2]=self.quantize(tensor[mask==2], input_compress_settings={'n':4})
                vec_q[mask==4]=self.quantize(tensor[mask==4], input_compress_settings={'n':16})
                vec_q[mask==6]=self.quantize(tensor[mask==6], input_compress_settings={'n':64})
                vec_q[mask==8]=self.quantize(tensor[mask==8], input_compress_settings={'n':256})
